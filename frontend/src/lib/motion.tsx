 ...
 * element crosses the viewport threshold, then stops observing. */
 ...
 * measures a tall section against its own scrollable length, which is what a
 * sticky/pinned child needs. */
 ...
 * IntersectionObserver: one section (the pinned pipeline) is many viewports
...
/** Editorial line-by-line reveal: each word lifts in sequence. Used for the
...
/** Reveals lines one at a time once in view — drives the terminal/log
 * playback. Returns how many lines should currently be visible. */
export function useLinePlayback(
  total: number,
  stepMs = 260,
): [(n: HTMLElement | null) => void, number] {
...
/** Cursor-driven 3D tilt with spring-damped easing. Writes to CSS custom
 * properties only (--rx/--ry rotation, --gx/--gy specular highlight, --sx/--sy
 * shadow offset, --lift proximity elevation) so painting stays on the CSS
 * side and there's no layout thrash. No-ops for reduced-motion / touch. */
